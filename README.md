...
클라이언트(Android)에서 일관된 에러 처리를 하기 어려움.
...
응답 JSON이 일관적(code, message).

📌 3. CustomException 통합

모든 비즈니스 예외는 이제 throw new CustomException(RsCode.XXX) 형태로 던짐.

GlobalExceptionHandler 는 딱 하나의 @ExceptionHandler(CustomException.class) 만 두고, 여기서 RsCode 값을 읽어 클라이언트에 내려줌.

그 외 스프링/인증/검증 관련 기본 예외(MethodArgumentNotValidException, AuthenticationException 등)는 별도로 처리.
...
Android, 웹 프론트 등 클라이언트는 code 기준으로 분기 처리 가능.
